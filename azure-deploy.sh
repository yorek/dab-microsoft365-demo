...
az storage account create -n $STORAGE_ACCOUNT -g $RESOURCE_GROUP --allow-blob-public-access false --sku Standard_LRS \
	-o json >> log.txt	
...
az webapp create -g $RESOURCE_GROUP -p "$APP_PLAN_ID" -n $APP_NAME -i "$IMAGE_NAME" \
    -o json >> log.txt
...
echo "configure webapp appsettings" | tee -a log.txt
az webapp config appsettings set -g $RESOURCE_GROUP -n $APP_NAME --settings AZURE_SQL=$AZURE_SQL \
    -o json >> log.txt
